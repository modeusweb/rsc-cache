 ...
 *
 * Why hand-rolled? Key derivation must be synchronous (so `cachedFn.key()` is
 * sync and keys are stable), available on every runtime (Node, Bun, Deno,
 * Cloudflare Workers, browsers) and free of imports. `crypto.subtle` is async
...
/** Computes the SHA-256 digest of a string (UTF-8) or byte array. */
export function sha256(input: string | Uint8Array): Uint8Array {
  const message = typeof input === "string" ? textEncoder.encode(input) : input;
...
/** Lowercase hex digest of SHA-256. */
